...
- Proved client-independent expiry with a separate no-producer realtime stack: a one-shot Scheduler target invoked `ExpiryReaper` at `2026-09-12T08:38:13Z`; its CloudWatch log recorded `outcome: deleted` at `08:38:47Z`, and the stack/schedule were absent afterward. Final post-batch scoped inventory found core only, no Kinesis/schedules, and 2,039 retained S3 objects / 2,134,080 bytes. Export/reset/recreate/restore remains a deliberately unrun destructive acceptance drill.
