...
| M3 Integrated batch | M1 + M2 raw contract / AWS accepted | CSV/JSON union, pump join, DQ + dedupe, immutable run-scoped output, publication pointer, date backfill and gold KPI implementation; all five acceptance proofs are recorded in the runbook |
| M4 Operations/security | M2 + M3 / AWS partial acceptance | Cloud batch completion, scoped retry/catch, publish gate, expiry lease, drain, least privilege, denial simulation, export/restore tooling and residual cost inventory; live drain, expiry and denial proof recorded; destructive export/reset/restore drill remains required |
