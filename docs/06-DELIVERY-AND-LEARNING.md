...
| M4 Operations/security | M2 + M3 / local implementation complete; AWS acceptance pending | Cloud batch completion, scoped retry/catch, publish gate, expiry lease, drain, least privilege, denial simulation, export/restore tooling and residual cost inventory; live laptop-loss/deny/restore evidence remains required |
