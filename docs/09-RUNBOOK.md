...
> **AWS accepted on 2026-09-13.** An independent M4 Step Functions execution (`433ea1a3-19b7-43df-b5bc-a17add0c0950`) ran Glue successfully, then started the crawler itself; it reached `SUCCEEDED` at `15:47:19 +07:00`, with crawler `READY` and a fresh successful crawl at `15:46:18 +07:00`. G4 returned `implicitDeny` for all five prohibited actions. A normal one-minute realtime run reconciled 180 acknowledged, 180 raw, 0 quarantine and 0 missing. Scheduler expiry independently deleted a realtime stack. On 2026-09-13, two destructive export/reset/recreate/restore drills each restored 2,039 S3 objects / 2,134,080 bytes and 3/0 DynamoDB items; the verifier read every restored object for SHA-256 comparison and compared canonical table content with its export manifest. The final account state retained core only, with no Kinesis streams or Scheduler schedules.
